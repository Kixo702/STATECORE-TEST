...
const isUserOnline = (u) => {
  const ts = u?.lastSeen ? new Date(u.lastSeen).getTime() : NaN
...
const IC = {
  ...
  users:   <svg viewBox="0 0 24 24" className="w-5 h-5" fill="none"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"/><circle cx="9" cy="7" r="4" stroke="currentColor" strokeWidth="1.7"/><path d="M23 21v-2a4 4 0 0 0-3-3.87" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"/><path d="M16 3.13a4 4 0 0 1 0 7.75" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"/></svg>,
...
export default function Dashboard({ user, onLogout, onOpenProfile }) {
  ...
  // ── Пользователи онлайн (реальные, по heartbeat lastSeen с бэкенда) ──
  ...
  useEffect(() => {
    ...
    const loadOnline = async () => {
      try {
        ...
        if (cancelled) return
        setOnlineUsers(users.filter(isUserOnline))
      } catch (e) {
        console.error(e)
      } finally {
  ...
  return (
    <div className="text-white min-h-screen" style={{ background: 'radial-gradient(circle at 12% 0%, #1a2440 0%, #0a0e18 50%)' }}>
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 md:px-10 py-8 sm:py-10">
        ...
        <div className="flex items-center gap-3 mb-4">
          <span className="text-[11px] font-extrabold tracking-[2.5px] uppercase text-white/35">
            Пользователи онлайн {!loadingOnline && `(${onlineUsers.length})`}
          </span>
          <div className="flex-1 h-px bg-white/5" />
        </div>
        ...
        {loadingOnline ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 mb-10">
            {[0, 1, 2].map(i => (
              <div key={i} className="rounded-xl border border-white/[0.08] bg-white/[0.015] p-5">
                <div className="skeleton-text h-5 w-32 mb-2.5" />
                <div className="skeleton-text h-3.5 w-20" />
              </div>
            ))}
          </div>
          ...
          >
            <div className="flex flex-col items-center justify-center text-center px-6 py-12">
              ...
              <h3 className="text-lg font-black text-white mb-1.5">Сейчас никого нет онлайн</h3>
              <p className="text-sm text-slate-400 max-w-md">Список обновляется автоматически по мере активности пользователей</p>
            </div>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 mb-10">
            {onlineUsers.map((u, i) => (
              <button
                ...
                className="group relative overflow-hidden rounded-xl border border-white/[0.08] bg-white/[0.015] hover:bg-white/[0.035] hover:border-white/[0.16] transition-all duration-300 text-left disabled:cursor-default disabled:hover:bg-white/[0.015]"
                style={{ animation: `db-statPop .3s cubic-bezier(.34,1.4,.64,1) both`, animationDelay: `${i * 50}ms` }}
              >
                <div className="absolute left-0 top-0 bottom-0 w-[3px] bg-emerald-400/70" />
                <div className="flex items-center gap-3 pl-5 pr-4 py-4">
                  <div className="relative w-10 h-10 rounded-xl flex items-center justify-center shrink-0 bg-emerald-400/10 text-emerald-300 font-black text-sm">
                    {(u.nickname || u.name || u.login || '?')[0]?.toUpperCase()}
                    <span className="absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full bg-emerald-400 border-2 border-[#0a0e18]" />
                  </div>
                  <div className="min-w-0">
                    <h3 className="font-bold text-sm truncate">{u.nickname || u.name || u.login || 'Без имени'}</h3>
                    <p className="text-xs text-emerald-300/70 flex items-center gap-1.5 mt-0.5">
                      <span className="w-1.5 h-1.5 flex items-center justify-center">{IC.dot}</span>
                      Онлайн
                    </p>
                  </div>
                </div>
