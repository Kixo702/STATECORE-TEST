...
// Пользователь считается "онлайн", если его heartbeat (lastActive) укладывается
// в это окно. Heartbeat шлётся из App.jsx каждые ~25с, поэтому 90с даёт запас
// на пропущенный тик/задержку сети, не показывая при этом давно ушедших.
...
export default function Dashboard({ user, onLogout, onOpenProfile }) {
  ...
  // Пользователи онлайн — считаем таковыми всех, чей lastActive (heartbeat из
  // App.jsx, обновляется каждые ~25с, пока сайт открыт) укладывается в окно
  // ONLINE_WINDOW_MS. Список подгружается с бэкенда и обновляется по опросу.
  ...
  // Опрашиваем бэкенд за списком пользователей и фильтруем по свежести
  // lastActive — так блок "Онлайн" отражает реальных людей на сайте прямо
  // сейчас, а не заглушку.
  useEffect(() => {
    let cancelled = false
    const loadOnline = async () => {
      try {
        ...
        const online = (Array.isArray(users) ? users : [])
          .filter(u => u.lastActive && (nowTs - new Date(u.lastActive).getTime()) < ONLINE_WINDOW_MS)
  ...
  return (
    <div className="text-white min-h-screen" style={{ background: 'radial-gradient(circle at 12% 0%, #1a2440 0%, #0a0e18 50%)' }}>
      ...
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 md:px-10 py-8 sm:py-10">
        ...
        <div className="flex items-center gap-3 mb-4">
          <span className="text-[11px] font-extrabold tracking-[2.5px] uppercase text-white/35">
            Пользователи онлайн{!loadingOnline && onlineUsers.length > 0 ? ` (${onlineUsers.length})` : ''}
          </span>
          <div className="flex-1 h-px bg-white/5" />
        </div>
        ...
        <div
          className="relative overflow-hidden rounded-2xl border border-white/[0.08] bg-white/[0.015] mb-10"
          style={{ minHeight: loadingOnline || onlineUsers.length === 0 ? 140 : 'auto' }}
        >
          {loadingOnline ? (
            <div className="flex flex-col items-center justify-center text-center px-6 py-14">
              <span className="w-6 h-6 text-orange-300/70" style={{ animation: 'db-spin .8s linear infinite' }}>{IC.spin}</span>
              <p className="text-sm text-slate-400 mt-4">Загрузка списка онлайн…</p>
            </div>
          ) : onlineUsers.length === 0 ? (
            <div className="flex flex-col items-center justify-center text-center px-6 py-14">
              <div className="w-14 h-14 rounded-2xl flex items-center justify-center mb-4 bg-white/5 text-orange-300/80">
                {IC.user}
              </div>
              <h3 className="text-lg font-black text-white mb-1">Сейчас никого нет на сайте</h3>
              <p className="text-sm text-slate-400 max-w-md">Как только кто-то откроет STATECORE, он появится здесь</p>
            </div>
          ) : (
            <div className="flex flex-wrap gap-3 p-4 sm:p-5">
              {onlineUsers.map((ou, i) => {
                const initial = (ou.nickname || ou.login || '?')[0]?.toUpperCase() || '?'
                return (
                  <button
                    key={ou.id || ou.login || i}
                    onClick={() => onOpenProfile && ou.id && onOpenProfile(ou.id)}
                    disabled={!onOpenProfile || !ou.id}
                    className="group flex items-center gap-3 pl-2.5 pr-4 py-2 rounded-xl border border-white/[0.08] bg-white/[0.02] hover:bg-white/[0.05] hover:border-orange-400/30 transition-all duration-200 text-left disabled:cursor-default disabled:hover:bg-white/[0.02] disabled:hover:border-white/[0.08]"
                    style={{ animation: `db-fadeUp .35s ease both`, animationDelay: `${i * 30}ms` }}
                    title={ou.roleName || 'Игрок'}
                  >
                    <div className="relative shrink-0">
                      <div className="w-9 h-9 rounded-lg overflow-hidden flex items-center justify-center text-xs font-black bg-gradient-to-br from-orange-500/25 to-white/[0.03] text-orange-200 border border-white/10">
                        {ou.avatar ? <img src={ou.avatar} alt={ou.nickname} className="w-full h-full object-cover" /> : initial}
                      </div>
                      <span className="absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full bg-emerald-400 border-2 border-[#0f1420]" title="В сети" />
                    </div>
                    <div className="min-w-0">
                      <div className="text-sm font-bold text-white truncate max-w-[140px] group-hover:text-orange-200 transition-colors">{ou.nickname || ou.login}</div>
                      <div className="text-[11px] text-slate-400 truncate max-w-[140px]">{ou.roleName || 'Игрок'}</div>
                    </div>
                  </button>
                )
              })}
